...
from typing import List, Set, Dict, Tuple, Iterable, TypeVar, Generic, Any
...
class FA_simple(object):
	...
	def write_FSM(self, filename):
		...
		sortedtranslist = sorted([x for x in self.transitionList], key=itemgetter(0,1,2))
	...
	def write_FSM_init(self, filename, states_excluded = []):
		...
		init_states = [str(x) for x in range(states_list_len) if str(x) not in states_excluded]
	...
	def read_FSM(filename):
		...
		if inp_num_check != fsm.numberOfInputs \
	...
	def read_FA(filename):
		...
		for line in fsm_list[:4]:
			splitted = line.strip().split(' ')
			info[splitted[0]] = int(splitted[1]) if len(splitted) == 2 else [int(x) for x in splitted[1:]]
	...
	def get_ns_out(self, state: int, inp: int) -> tuple[int, int]:
		...
		"""
		for tr in self.transitionList:
			if str(tr[0]) == str(state) and str(tr[1]) == str(inp): # TODO убрать приведение к str
				return (tr[2], tr[3])
		raise Exception(f"get_ns_out error: no such (state, input) = ({state}, {inp}) in the FSM")
	...
	def rename_inputs(self, from_to_dict: dict[int | str, int | str]) -> None:
		...
		"""
		assert len(set(from_to_dict.keys())) == self.numberOfInputs, "Cannot rename inputs"

		new_tr_list: list[Sequence[int | str]] = []
		for transition in self.transitionList:
			new_tr_list.append((transition[0], from_to_dict[transition[1]], transition[2], transition[3]))
	...
	def encode_states(self, is_abstraction: bool = False, forced_transform: bool = False) -> tuple[bool, dict[int, str], dict[int, int]]:
		...
		is_ok = 1
		if type(self.transitionList[0][0]) != int or not self.check_states_for_consistency():
			is_ok = 0
		else:
		...
		if self.isFSM:
			self.transitionList = [(str(x[0]), str(x[1]), str(x[2]), str(x[3])) for x in self.transitionList]
		else:
			self.transitionList = [(str(x[0]), str(x[1]), str(x[2])) for x in self.transitionList]
		...
		if self.initialState.isdigit():
			...
			abs_Intstate_to_abs_State_reversed[self.initialState] = int(self.initialState)
		else:
		...
		if self.isFSM == 1:
			self.transitionList = [(abs_Intstate_to_abs_State_reversed[x[0]], x[1], abs_Intstate_to_abs_State_reversed[x[2]], x[3]) for x in self.transitionList]
		else:
			self.transitionList = [(abs_Intstate_to_abs_State_reversed[x[0]], x[1], abs_Intstate_to_abs_State_reversed[x[2]]) for x in self.transitionList]
	...
	def encode_inputs_outputs(self, forced_transform: bool = False, dont_change_original:bool = False) -> tuple[bool | "FA_simple", dict, dict]: #uyB3sWj2s9J
		...
		for i in range (len(self.transitionList)):
			if type(self.transitionList[i][1]) != int \
		...
		if fsm.isFSM == 1:
			fsm.transitionList = [(x[0], new_input2abs_input_reversed[x[1]], x[2], new_output2abs_output_reversed[x[3]]) for x in fsm.transitionList]
		else:
			fsm.transitionList = [(x[0], new_input2abs_input_reversed[x[1]], x[2]) for x in fsm.transitionList]
	...
	def accept_FA(self, input_seq):
		...
		for inp in input_seq:
			for i in range(len(self.transitionList)):
				if str(self.transitionList[i][0]) == str(current_state) and str(self.transitionList[i][1]) == str(inp):
